...
将系统中所有面向用户/管理员的"杏林食速"品牌痕迹替换为"杏林食速"，使系统符合南通大学杏林学院校园外卖的场景定位。
...
| 阶段 | 文件路径 | 替换内容 | 说明 |
|------|---------|---------|------|
| 1 | `core/backend/sky-server/src/main/resources/application-dev.yml` | `shop-name: 杏林食速` → `杏林食速` | 后端店铺名配置，驱动前端动态展示 |
| 1 | `core/backend/sky-common/src/main/java/com/sky/properties/StorefrontProperties.java` | `private String shopName = "杏林食速"` → `"杏林食速"` | 属性类默认值 |
| 1 | `core/backend/sky-server/src/main/java/com/sky/config/WebMvcConfiguration.java` | `"杏林食速项目接口文档"` → `"杏林食速项目接口文档"`（4处） | Swagger/Knife4j 文档标题与描述 |
| 2 | `core/miniapp/pages/index/file.vue` | `name: "苍穹"` → `"杏林食速"` | 小程序端品牌展示 |
| 2 | `core/miniapp/README.md` | 标题中的"杏林食速" → "杏林食速" | 小程序文档 |
| 2 | `core/miniapp/小程序开发流程.md` | 标题中的"杏林食速" → "杏林食速" | 小程序文档 |
| 3 | `core/nginx/html/sky/merchant-admin/联调指南.md` | "杏林食速" → "杏林食速" | 前端联调文档 |
| 4 | `README.md` | "杏林食速" → "杏林食速" | 项目主文档 |
| 4 | `SETUP.md` | "杏林食速" → "杏林食速" | 开发环境文档 |
| 4 | `.env.example` | "杏林食速" → "杏林食速" | 环境变量模板 |
| 4 | `startup-report.md` | "杏林食速" → "杏林食速" | 启动报告 |
| 4 | `docs/superpowers/plans/2026-05-04-jwt-blacklist.md` | `shop-name: 杏林食速` → `杏林食速` | 计划文档配置示例 |
| 4 | `docs/superpowers/agent-briefs/2026-05-05-agent-05-devops-engineering.md` | "杏林食速" → "杏林食速" | Agent 简报 |
| 4 | `docs/deployment/database-migrations.md` | "杏林食速" → "杏林食速" | 部署文档 |
| 4 | `docs/deployment/troubleshooting.md` | "杏林食速" → "杏林食速" | 故障排查文档 |
| 4 | `docs/thesis-upgrade/staged-content.md` | "杏林食速" → "杏林食速" | 论文升级文档 |
...
**验证方式：**
- 使用 `grep -r "杏林食速" --include="*.md" --include="*.yml" --include="*.txt" D:/sky-delivery` 确认核心源文件中无残留
...
7. **残留扫描**：全部阶段完成后，执行全局 grep 扫描，确认核心源文件（排除 build 产物）中无"杏林食速"残留
...
- [ ] 小程序首页品牌名显示"杏林食速"
- [ ] 全局 grep 扫描核心源文件无"杏林食速"残留（排除 target/、worktrees/、node_modules/）
- [ ] 数据库连接正常，业务功能无异常
