...
export default {
	...
	methods: {
		...mapState(['orderListData', 'remarkData', 'addressData', 'storeInfo', 'shopInfo', 'deliveryFee', 'currentMerchantId']),
		...
		buildMerchantParams(params = {}) {
			return withMerchantScope(params, this.getCurrentMerchantId())
		},
		init() {
		...
		async getEstimatedDeliveryTime() {
			const result = await getEstimatedDeliveryTime(this.buildMerchantParams({ customerAddress: this.address }));
		...
		payOrderHandle() {
			...
			submitOrderSubmit(this.buildMerchantParams(params)).then(res => {
				if (res.code === 1) {
