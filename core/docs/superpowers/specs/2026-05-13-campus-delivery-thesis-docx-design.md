...
本项目从设计、写作、模板回填到最终交付，最高优先级指令是：尽可能避免大模型幻觉，并且全程遵循这条指令。

这条指令高于行文完整性、高于篇幅充实度、高于“看起来像一篇完整论文”的表面完成度。只要某个事实、数据、接口、字段、测试结果、文献条目、图表说明或结论没有被当前已确认材料直接支撑，就不能把它写成确定事实。

执行时必须遵守以下落地规则：

- 只允许把“用户明确确认过的信息”或“已经登记到 workflow/evidence/源码/SQL/测试/截图中的信息”写成确定表述。
- 现有初稿和旧文案只能作为语言素材来源，不能单独作为事实来源；凡是旧稿里的关键事实，都要尽量回指到源码、SQL、接口、测试或截图。
- 遇到无法验证但又可能重要的内容，优先选择“降级表述、显式保留限制或直接不写”，而不是猜测补全。
- 如果某一章节想成立必须依赖缺失证据，则宁可让该处内容保守、简短或留在附件/限制说明里，也不允许编造。
- 在主论文和附件 `.docx` 中，不得虚构接口路径、数据库字段、测试结论、性能数据、生产数据、调研数据、创新点或参考文献。
- 如果模板或旧稿中的现成内容与已验证证据冲突，以已验证证据为准，并重写冲突部分。
...
- 质量边界：不使用渲染校验功能
- 最高指令：全程尽可能避免大模型幻觉，宁缺毋滥，不把未验证内容写成既成事实
...
- 更新 workflow 状态文件
- 先做一轮“反幻觉过滤”，把无法回指证据的旧稿事实标记为待删、待降级或待改写
...
- 对无法直接验证的段落，不追求补满篇幅，优先删减、改写为保守表述，或转入限制说明

### 第三步：构建附件中间稿

- 以表格和短说明为主
- 优先放入证据性强、正文不宜过长展开的材料
- 附件同样遵守反幻觉优先，不把“可能存在但未验证”的材料整理进去
